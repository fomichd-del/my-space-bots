...
def get_best_image_url(asset_manifest_url):
    """Выбирает качественное фото под лимит ТГ."""
    try:
...
def send_to_telegram():
    ...
    # --- ШАГ 1: Отправка фото БЕЗ кнопок для активации комментариев ---
    ...
    if r.status_code == 200:
        ...
        print(f"✅ Пост {nasa_id} отправлен. Активирую пульт управления...")
        
        # Небольшая пауза для стабилизации связи с группой обсуждения
        time.sleep(2)
        
        # --- ШАГ 2: Добавление кнопок через редактирование ---
        keyboard = {
            "inline_keyboard": [
