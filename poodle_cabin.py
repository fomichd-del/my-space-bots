...
def get_dog_image(dog, user_id):
    if dog['status'] == 'dead':
        return "https://image.pollinations.ai/prompt/" + urllib.parse.quote("empty dog bed, abandoned futuristic spaceship cabin, lonely atmosphere, realistic photographic style")
    
    # 🟢 ОСНОВА: РЕАЛИСТИЧНЫЙ ТОЙ-ПУДЕЛЬ
...
def send_dog_menu(bot, chat_id, user_id):
    ...
    url = get_dog_image(dog, user_id)
    
    if dog['status'] == 'dead':
        text = "🛰 **СИГНАЛ ПОТЕРЯН**\n\nКомандор, твой верный пес покинул корабль из-за плохого ухода. Каюта пуста..."
        kb = tele_types.InlineKeyboardMarkup()
        kb.add(tele_types.InlineKeyboardButton("🛰 Вызвать нового щенка (-100 💰)", callback_data="dog_resurrect"))
    else:
        text = (
            f"🐕 **КАЮТА ПИТОМЦА**\n\n"
...
def handle_dog_callback(bot, call):
    ...
    if action == "feed":
        ...
        dog['energy'] = 100
        update_dog_data(user_id, dog)
        bot.answer_callback_query(call.id, "💤 Пес сладко спит в регенерационной капсуле...")

    elif action == "play":
        if spend_dust(user_id, 5):
            dog['mood'] += 30; dog['energy'] -= 20
            update_dog_data(user_id, dog)
            bot.answer_callback_query(call.id, "🎾 Грави-мяч — это весело!")
        else: bot.answer_callback_query(call.id, "❌ Нужно 5 пыли!")

    elif action == "shop":
        text = "🛒 **КОСМИЧЕСКИЙ ГАРДЕРОБ**\n\nКупленные вещи пудель наденет сразу!"
        ...
        if spend_dust(user_id, 100):
            new_dog = {"level": 1, "hunger": 80, "energy": 80, "mood": 80, "items": [], "xp": 0, "date": "", "status": "alive"}
            update_dog_data(user_id, new_dog)
            bot.answer_callback_query(call.id, "🛰 Новый щенок прибыл на борт!")
