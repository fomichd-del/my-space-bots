...
save_vision_context, get_vision_context, clear_vision_context, get_dog_data, init_db, get_dog_profession, check_and_update_quiz, is_user_new)
...
def force_digest_test(message):
    ...
    try:
        ...
        # ⚡️ КИБЕР-ПРОТОКОЛ ДЛЯ МАРТИ (БЕЗ ГАЛЛЮЦИНАЦИЙ И ЛИШНИХ ССЫЛОК)
        prompt = (
            "Ты — бортовой компьютер Марти. Твоя задача: СТРОГОЕ ФОРМАТИРОВАНИЕ ДАННЫХ.\n"
            f"ВХОДНЫЕ ДАННЫЕ ДЛЯ ОБРАБОТКИ:\n{raw_text}\n\n"
            "СТРОЖАЙШИЕ ПРАВИЛА:\n"
            "1. Каждая новость содержит разделитель '|LINK|'. Текст ПОСЛЕ него — это единственно верная ссылка.\n"
            "2. Ты ДОЛЖЕН превратить каждую новость в Markdown-гиперссылку: [Суть новости](ССЫЛКА).\n"
            "3. ЗАПРЕЩЕНО выводить URL-адрес текстом. Он должен быть ПОЛНОСТЬЮ СКРЫТ внутри квадратных скобок.\n"
            "4. ЗАПРЕЩЕНО использовать любые внешние ссылки или упоминать 'k_prostranstvo_bot'.\n"
            "5. Одна новость — одна строка с подходящим эмодзи.\n"
            "6. Стиль: бодрый, научный. В конце напиши 'Прием!'"
        )
...
def handle_channel_post(message):
    try:
        if str(message.chat.id) == str(CHANNEL_ID) and (message.text or message.caption):
            ...
            # 🧹 САНИТАРНАЯ ОБРАБОТКА ТЕКСТА
            # Убираем символ '|', чтобы ИИ не путал его с нашим разделителем |LINK|
            clean_text = text.replace('|', '').strip()
            
            # Сокращаем слишком длинные посты, чтобы промпт не «пух»
            ...
            # Формируем запись: "Текст новости |LINK| прямая_ссылка"
            news_entry = f"{clean_text} |LINK| {news_link}"
        ...
        while True:
            ...
            if "15:00" <= current_time_utc <= "15:15" and last_sent_date != yesterday_date:
                ...
                if news:
                    ...
                    # ⚡️ КИБЕР-ПРОТОКОЛ ДЛЯ МАРТИ (БЕЗ ГАЛЛЮЦИНАЦИЙ)
        prompt = (
            "Ты — бортовой компьютер Марти. Твоя задача: СТРОГОЕ ФОРМАТИРОВАНИЕ ДАННЫХ.\n"
            f"ВХОДНЫЕ ДАННЫЕ:\n{raw_text}\n\n"
            "АЛГОРИТМ ДЕЙСТВИЙ:\n"
            "1. Каждая новость в списке разделена маркером '|LINK|'.\n"
            "2. Текст ДО маркера — это описание. Текст ПОСЛЕ маркера — это ССЫЛКА.\n"
            "3. Ты должен создать Markdown-строку: '[Эмодзи] [Короткая суть новости](ССЫЛКА)'.\n"
            "4. СТРОЖАЙШЕ ЗАПРЕЩЕНО использовать любые ссылки, кроме тех, что даны после '|LINK|'.\n"
            "5. ЗАПРЕЩЕНО писать ссылки текстом (в скобках или без). Только гиперссылки на словах.\n"
            "6. ЗАПРЕЩЕНО упоминать 'k_prostranstvo_bot' или любые другие сторонние ресурсы.\n"
            "7. Формат вывода: Список новостей через разделитель ⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯.\n"
            "8. В конце напиши: 'Прием!'"
        )
