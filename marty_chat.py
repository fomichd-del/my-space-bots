...
def force_digest_test(message):
    ...
    try:
        ...
        # ⚡️ КИБЕР-ПРОТОКОЛ "СТРЕЛКА ОРИОНА"
        prompt = (
            "Ты — Марти, ученый пес-пудель. Твоя задача: СТРОГОЕ ОФОРМЛЕНИЕ ДАЙДЖЕСТА.\n"
            f"ДАННЫЕ:\n{raw_text}\n\n"
            "ПРАВИЛА ОФОРМЛЕНИЯ:\n"
            "1. Каждая новость — это ОДНА строка.\n"
            "2. В начале строки — тематический эмодзи.\n"
            "3. Далее — краткая суть новости (максимум 10-12 слов).\n"
            "4. В САМОМ КОНЦЕ каждой новости поставь символ ' ➡ ' или ' 🛰 ' и сделай его ГИПЕРССЫЛКОЙ на адрес, который указан после '|LINK|'.\n"
            "5. ПРИМЕР СТРОКИ: 🪐 Открыта новая туманность в созвездии Ориона! [➡](ссылка)\n"
            "6. СТРОЖАЙШЕ ЗАПРЕЩЕНО использовать любые сторонние ссылки. Только те, что идут после '|LINK|'.\n"
            "7. Текст новости НЕ должен быть ссылкой. Ссылкой должен быть ТОЛЬКО символ стрелочки в конце.\n"
            "8. Тон: бодрый, космический. В конце: 'Прием!'"
        )
