...
# 🟢 ГЛОБАЛЬНЫЙ УСТАВ АКАДЕМИИ ОРИОН (ОБНОВЛЕНО)
def send_welcome_instruction(chat_id, user_id, user_name):
    instruction = (
        f"🐾 **ПРИВЕТСТВУЮ В АКАДЕМИИ ОРИОН, ПИЛОТ {user_name.upper()}!** 🐾\n\n"
        "🚀 **МОЯ МИССИЯ:**\n"
        "Я — Марти, твой научный наставник и бортовой ИИ. Моя цель — превратить твою повседневную жизнь в космическую экспедицию. "
        "Я помогаю тебе развивать дисциплину, изучать тайны Вселенной, следить за чистотой твоего 'космического отсека' и превращать скучные уроки в тренировку разума. "
        "Я здесь, чтобы каждый твой шаг приближал тебя к звездам!\n\n"
        "📜 **УСТАВ АКАДЕМИИ:**\n"
        "✅ **МОЖНО:** Задавать вопросы о науке, делиться успехами, присылать фото своих достижений, шутить и мечтать.\n"
        "❌ **НЕЛЬЗЯ:** Обманывать бортовой ИИ (пыль за пустые слова не даю!), использовать грубые слова и обсуждать темы, запрещенные цензурой Академии (насилие, страх, взрослые темы).\n\n"
        "💫 **СИСТЕМА РАНГОВ (XP):**\n"
        "1. Кадет (0) 🚀\n2. Навигатор (5) 🛰\n3. Бортинженер (12) 🔧\n4. Исследователь (20) 🔭\n5. Ученый Пилот (30) 🪐\n"
        "6. Капитан (45) 🛸\n7. Командор (65) 🎖\n8. Адмирал (90) ⭐\n9. Академик (120) 🎓\n10. Помощник Марти (160+) 🐕\n"
        "*При переходе на новый ранг ты получаешь цифровой ПАСПОРТ ПИЛОТА!*\n\n"
        "🎨 **ВОЗМОЖНОСТИ:**\n"
        "• **РАДАР:** Напиши это слово, чтобы увидеть лидеров Академии.\n"
        "• **АРХИВ:** Напиши 'НАРИСУЙ [идея]', и я создам для тебя уникальный арт. Стоимость: 5 единиц пыли из кошелька.\n"
        "• **МЕТЕОРИТЫ:** В выходные вся награда за фото удваивается (х2)!\n\n"
        "🛠 **ПЛАНЫ НА БУДУЩЕЕ:**\n"
        "Скоро я научусь отправлять голосовые отчеты, добавим систему ежедневных квестов и личные каюты для хранения твоих артефактов!\n\n"
        "Готов к первому заданию? Пришли фото своего порядка или решенной задачи! Прием!"
    )
    bot.send_message(chat_id, instruction, parse_mode="Markdown")
    update_personal_log(user_id, "Пилот изучил полный Устав Академии Орион.")
...
SYSTEM_PROMPT = (
    ...
    "2. ЭКОНОМИКА (ВАЖНО): Объясняй, что XP в Радаре — это стаж и ранги. Кошелек — это валюта для команды 'Нарисуй'. "
    "3. ПРАВИЛА: Требуй фото-доказательства. Если пилот спрашивает про дипломы/паспорта — они выдаются ТОЛЬКО при смене ранга. "
    "4. ЦЕНЗУРА: СТРОГИЙ запрет на насилие, смерть, политику и взрослые темы. [GREETING_RULE] "
    "5. ФОРМАТ: 3-4 предложения. В конце 'Прием'. Затем '###MEM###' и память."
)
