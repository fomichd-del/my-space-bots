...
def generate_star_map(lat, lon, user_name, user_id):
    ...
    try:
        ...
        # РЕЗОЛЮЦИЯ 1000 (Золотая середина)
        ...
        sky_size = 920 # Оптимальный размер для центровки
        ...
        fig.text(0.35, 0.106, f"Фаза: {moon_phase}%", color=t_col, fontsize=14, fontweight='bold')
        fig.text(0.35, 0.067, rise_time, color=t_col, fontsize=14, fontweight='bold')
        ...
        # Сохранение сразу в JPG
        plt.savefig(final_path, format="jpg", bbox_inches='tight', pad_inches=0, dpi=dpi, quality=85)
