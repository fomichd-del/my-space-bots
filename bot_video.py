...
async def process_video_async(video_url):
    ...
    clear_workspace()
    try:
        ydl_opts = {'format': 'best[height<=720][ext=mp4]', 'outtmpl': f_in, 'quiet': True, 'noplaylist': True, 'ignoreerrors': True}
        dur = 0
...
async def main():
    print("🎬 [ЦУП] v37.0 'Supermassive Shield' запущен...")
        ...
        try:
            ...
            if s['t'] == 'nasa_api':
                try:
                    ...
                    items = nasa_res.get('collection', {}).get('items', [])
                    for item in items[:5]:
                        v_id = item['data'][0]['nasa_id']
                        if f"nasa_{v_id}" in db: continue
                        assets_res = requests.get(f"https://images-api.nasa.gov/asset/{v_id}").json()
                        v_url = next(a['href'] for a in assets_res['collection']['items'] if '~medium.mp4' in a['href'])
            ...
            for v in video_list:
                ...
                # ВСЕГДА ЗАПИСЫВАЕМ В БАЗУ, ЧТОБЫ ИЗБЕЖАТЬ ПЕТЛИ ОШИБОК
                if mode == "error" or not path:
