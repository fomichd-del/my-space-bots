...
async def build_voice_track(segments):
    ...
    for i, seg in enumerate(segments[:75]):
        try:
...
async def process_video_async(video_url, is_yt):
    ...
    try:
        ...
        # ПРОВЕРКА НАЛИЧИЯ АУДИО В ОРИГИНАЛЕ
        has_audio = False
        try:
            audio_check = subprocess.check_output(f"ffprobe -i {f_in} -show_streams -select_streams a -loglevel error", shell=True)
            if audio_check: has_audio = True
        ...
        try:
            ...
            for item in items[:3]:
                ...
                if link and link not in db:
                    ...
                    title = t_node.text if t_node is not None else "Событие"
                    ...
                    d_ru = super_clean(translator.translate(desc[:300])) if desc else "Репортаж из космоса."
