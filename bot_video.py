...
async def build_voice_track(segments, total_duration):
    print("🎙 [АУДИО] Начинаю синтез речи...")
...
async def process_video_async(video_url, is_yt):
    print(f"🎬 [МОНТАЖ] Обработка: {video_url}")
...
async def main():
    print("🎬 [ЦУП] v21.0 'Deep Scan' запущен...")
    db = open(DB_FILE, 'r').read() if os.path.exists(DB_FILE) else ""
    print(f"📂 [БАЗА] Записей в истории: {len(db.splitlines())}")
        ...
        try:
            ...
            for item in items[:2]:
                ...
                if s['t'] == 'nasa_api':
                    ...
                    link = next(a['href'] for a in assets['collection']['items'] if '~medium.mp4' in a['href'])
                    title = item['data'][0]['title']
                    desc = item['data'][0].get('description', '')
                    v_id_save = f"nasa_{v_id}"
                else:
                    link = item.find('link').text if item.find('link') is not None else ""
                    if link in db:
                        print(f"   --- Видео {link[:30]} уже было. Пропускаю.")
                        continue
                    title = item.find('title').text
                    desc = item.find('description').text if item.find('description') is not None else ""
                    v_id_save = link

                print(f"🚀 [ЗАПУСК] Найдено новое видео! Начинаю процесс...")
                path, mode = await process_video_async(link, 'youtube' in link)
                if not path: continue

                caption = (f"⭐ <b>{super_clean(safe_translate(title)).upper()}</b>\n\n🪐 <b>ИСТОЧНИК:</b> {s['n']}\n🔊 <b>ЗВУК:</b> {('Перевод' if mode=='voice' else 'Оригинал')}\n─────────────────────\n📖 <b>СЮЖЕТ:</b> {super_clean(safe_translate(desc[:350]))}...\n\n🚀 <a href='https://t.me/vladislav_space'>Дневник юного космонавта</a>")

                with open(path, 'rb') as v:
                    r = requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendVideo", files={"video": v}, data={"chat_id": CHANNEL_NAME, "caption": caption, "parse_mode": "HTML"}, timeout=180)
                
                if r.status_code == 200:
                    with open(DB_FILE, 'a') as f: f.write(f"\n{v_id_save}")
                    print("🎉 [ФИНИШ] Опубликовано!"); return
                else:
