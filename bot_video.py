...
print("🚀 [ЦУП] Системы инициализированы. Запуск v160.3 'Event Horizon Plus'...")

# ============================================================
# ⚙️ КОНФИГУРАЦИЯ (Stability + Power + Beauty)
# ============================================================
...
SAFE_LIMIT_MB  = 46  # Повышенный лимит качества

# Ключевые слова для фильтрации контента (если нужно)
...
def get_smart_summary(text):
    """Очистка текста от ссылок, хештегов и рекламы"""
...
def get_fast_proxy():
    """Поиск рабочего прокси-коридора"""
...
async def process_mission(v_id, title, desc_raw, is_russian=False, source_name=""):
    ...
    try:
        ...
        # УМНАЯ НАВИГАЦИЯ (Выбор разрешения)
        h_limit = 720
        ...
        # --- 3. МОЗГОВОЙ ЦЕНТР (Whisper) ---
        has_subs, mode_tag = False, "🎙 ОРИГИНАЛЬНАЯ ОЗВУЧКА"
        if not is_russian:
            if whisper_model is None:
                print("🧠 [ЦУП] Загрузка нейропрофиля Whisper...")
        ...
        print(f"⚙️ Сжатие: {raw_mb:.1f}Мб -> {SAFE_LIMIT_MB}Мб (Битрейт: {v_br})")
        subprocess.run([
...
async def main():
    ...
    for s in SOURCES:
        ...
        try:
            ...
            p_items = requests.get(f"https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId={up_id}&maxResults=3&key={YOUTUBE_API_KEY}").json()
            vids = [{'id': i['snippet']['resourceId']['videoId'], 'title': i['snippet']['title'], 'desc': i['snippet']['description']} for i in p_items['items']]
