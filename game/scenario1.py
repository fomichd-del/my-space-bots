...
def run_scenario(bot, call):
    ...
    # --- [ АНТИ-ФАРМ СИСТЕМА ] ---
    saved_flags = ""
    for flag in ["_ch1_claimed", "_item_moss"]:
        if flag in current_node:
            saved_flags += flag

    # 0. ПРОВЕРКА ЗАВЕРШЕНИЯ ГЛАВЫ
    is_finished = any(mark in current_node for mark in ["chapter1_finished", "chapter1_gold_finished"])
    ...
    if call.data == "game_start":
        ...
        if current_node and current_node not in ["start", "shluz_entry"] and not current_node.startswith("start"):
            # --- [ АТМОСФЕРНЫЙ ЛОКАТОР ГЛАВЫ 1 ] ---
            if "hacking_panel" in current_node:
                location_text = "У панели шлюза 'Авалон-7'. Идет взлом."
            elif "calculating" in current_node:
                location_text = "Рубка управления. Терминал вычисляет траекторию."
            elif "corridor" in current_node or "unlocked" in current_node:
                location_text = "Главный коридор станции. Невесомость и хаос."
            elif "sector_zero" in current_node:
                location_text = "Сектор Зеро. Абсолютная темнота."
            elif "keycard" in current_node or "moss" in current_node:
                location_text = "У кокона капитана. Биологическая угроза."
            elif "secret_lab" in current_node:
                location_text = "Секретная Лаборатория. Контейнеры с пылью."
            else:
                location_text = "Шлюзовой отсек. Сенсоры сканируют темноту."

            text = (f"🛰 **БОРТОВОЙ ЖУРНАЛ: ГЛАВА 1**\n\n"
                    f"{username}, системы восстановили последний сеанс связи.\n"
                    f"📍 **Локация:** *{location_text}*\n\n"
                    f"Желаете продолжить миссию?")
            
            kb = tele_types.InlineKeyboardMarkup(row_width=1)
            
            if "hacking_panel" in current_node:
                kb.add(tele_types.InlineKeyboardButton("🔄 Проверить статус взлома", callback_data="game_check_hack"))
            elif "calculating_trajectory" in current_node:
                kb.add(tele_types.InlineKeyboardButton("🔄 Проверить расчеты", callback_data="game_check_trajectory"))
            elif "shluz_unlocked" in current_node:
                kb.add(tele_types.InlineKeyboardButton("🚶 Войти в коридор", callback_data="game_node_corridor"))
            elif "keycard_found" in current_node:
                kb.add(tele_types.InlineKeyboardButton("🧬 В Секретную Лабораторию", callback_data="game_node_secret_lab"))
            else:
                kb.add(tele_types.InlineKeyboardButton("🔄 Обновить статус", callback_data="game_start")) # Безопасный редирект
        ...
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "shluz_entry" + saved_flags)

    elif call.data == "game_reset":
        update_game_progress(user_id, "start" + saved_flags) # Сохраняем награды при сбросе
        bot.answer_callback_query(call.id, "Бортовой журнал очищен.")
        run_scenario(bot, type('obj', (object,), {'from_user': call.from_user, 'data': 'game_start', 'message': call.message}))
    ...
    # --- ВЕТКА ШКАФЧИКОВ ---
        ...
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "shluz_closet" + saved_flags)
    ...
    # --- ВЕТКА ПАНЕЛИ ---
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(
            tele_types.InlineKeyboardButton("🔄 Проверить готовность", callback_data="game_check_hack"),
            tele_types.InlineKeyboardButton("🏠 На мостик", callback_data="game_back_to_profile")
        )
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "hacking_panel" + saved_flags)
        ...
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "shluz_unlocked" + saved_flags)
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(
            tele_types.InlineKeyboardButton("🛰 Ввести координаты (если есть)", callback_data="game_node_panel_with_coords"),
            tele_types.InlineKeyboardButton("🔙 Вернуться в шлюз", callback_data="game_start")
        )
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")

    elif call.data in ["game_node_medalion", "game_node_marty_bring"]:
        ...
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "shluz_coords_found" + saved_flags)
        ...
        kb = tele_types.InlineKeyboardMarkup().add(
            tele_types.InlineKeyboardButton("🔄 Проверить результат", callback_data="game_check_trajectory"),
            tele_types.InlineKeyboardButton("🏠 На мостик", callback_data="game_back_to_profile")
        )
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "calculating_trajectory" + saved_flags)
        ...
        text = (f"✅ **РАСЧЕТ ЗАВЕРШЕН**\n\n"
                ...
                f"Там нет кислорода.")
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(tele_types.InlineKeyboardButton("🔦 Спуститься в Сектор Зеро", callback_data="game_node_sector_zero"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "sector_zero_entry" + saved_flags)
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(tele_types.InlineKeyboardButton("🔋 Выключить свет и замереть", callback_data="game_node_sector_zero"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(tele_types.InlineKeyboardButton("🩺 Теперь к кокону...", callback_data="game_node_open_cocoon"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(tele_types.InlineKeyboardButton("⚔️ Вскрыть кокон немедленно", callback_data="game_node_cut_cocoon"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        ...
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "keycard_found" + saved_flags)
        ...
        kb = tele_types.InlineKeyboardMarkup(row_width=1).add(tele_types.InlineKeyboardButton("🚀 Завершить Главу 1", callback_data="game_node_escape_chapter_gold"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        update_game_progress(user_id, "in_secret_lab" + saved_flags)
    ...
    # --- ФИНАЛЫ ГЛАВЫ С ЗАЩИТОЙ ОТ ПОВТОРНОГО НАЧИСЛЕНИЯ ---
        ...
        if "_ch1_claimed" not in current_node:
            ...
            update_game_progress(user_id, "chapter1_finished" + saved_flags + "_ch1_claimed")
            reward_text = "💰 Награда за прохождение: **20 Звездной пыли**."
        else:
            update_game_progress(user_id, "chapter1_finished" + saved_flags)
        ...
        if "_ch1_claimed" not in current_node:
            ...
            update_game_progress(user_id, "chapter1_gold_finished" + saved_flags + "_ch1_claimed")
            reward_text = "💰 Награда (с учетом секретов): **50 Звездной пыли**."
        else:
            update_game_progress(user_id, "chapter1_gold_finished" + saved_flags)
